...
function loadAuthScript() {
    if (!Magic.b_loaded_auth) {
        $.getScript("/tools/js/gold-login-cvp_magic_1.js")
...
function loadCVPPlayerScript() {
    if (!Magic.b_loaded_player) {
        $.getScript("/tools/js/cvp_watch_magic_1.js")
            .done(function(script, textStatus) {
                setPlayerParam();
                console.log("|-o-||-o-||-o-|", script, textStatus);
                Magic.b_loaded_player = true;
...
function init() {
    ...
    $(".thumb:eq(" + Magic.int_index + ")").addClass("current_video");
...
*/
    Magic.b_is_auth = (/([\d]+([\w])?){4,10}/).test($(".current_video p").attr("data_id"));
    Magic.b_is_cvp = ($(".current_video p").attr("data_id").length < 32);

    Magic.b_is_marathon = (Magic.b_is_cvp && (/preview/).test($(".current_video p").attr("data_id")));
